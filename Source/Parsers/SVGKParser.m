...
- (void) addParserExtension:(NSObject<SVGKParserExtension>*) extension
{
	...
	{
		[SVGKLogger logMessage:@"[%@] WARNING: attempted to add a ParserExtension that was already added = %@", [self class], extension];
		return;
	}
...
- (SVGKParseResult*) parseSynchronously
{
	...
	{
		[SVGKLogger logMessage:@"FATAL: attempting to run the parser twice in one thread; limxml is single-threaded only, so we are too (until someone wraps libxml to be multi-threaded)"];
	}
	...
	*/
	...
	/* 
	 SVGKLog(@"[%@] WARNING: Substituting entities directly into document, c.f. http://www.xmlsoft.org/entities.html for why!", [self class]);
	...
	*/
	...
	if( ctx ) // if libxml init succeeds...
	{
		...
		while( bytesRead > 0 )
		{
			...
			{
				[SVGKLogger logMessage:@"SVGKParser: 'cancel parse' discovered; bailing on this XML parse"];
				break;
			}
			else
			{
			...
			@catch( NSException* e )
			{
				[SVGKLogger logMessage:@"Exception while trying to parse SVG file, will store in parse results. Exception = %@", e];
			...
			if( [currentParseRun.errorsFatal count] > 0 )
			{
				...
				{
					[SVGKLogger logMessage:@"[%@] libXml reported internal parser error with magic libxml code = %li (look this up on http://xmlsoft.org/html/libxml-xmlerror.html#xmlParserErrors)", [self class], (long)libXmlParserParseError];
					currentParseRun.libXMLFailed = YES;
				}
				else
				{
					[SVGKLogger logMessage:@"[%@] SVG parser generated one or more FATAL errors (not the XML parser), errors follow:", [self class] ];
					for( NSError* error in currentParseRun.errorsFatal )
					{
						[SVGKLogger logMessage:@"[%@] ... FATAL ERRRO in SVG parse: %@", [self class], error];
					}
				}
...
- (SVGKSource *)loadCSSFrom:(NSString *)href
{
    ...
    if( cssSource == nil )
    {
        ...
        if( cssText == nil )
        {
			[SVGKLogger logMessage:@"[%@] Unable to find external CSS file '%@'", [self class], href];
        }
        else
        {
...
- (void)handleStartElement:(NSString *)name namePrefix:(NSString*)prefix namespaceURI:(NSString*) XMLNSURI attributeObjects:(NSMutableDictionary *) attributeObjects
{
...
#if DEBUG_XML_PARSER
			SVGKLog(@"[%@] tag: <%@:%@> id=%@ -- handled by subParser: %@", [self class], prefix, name, ([((Attr*)[attributeObjects objectForKey:@"id"]) value] != nil?[((Attr*)[attributeObjects objectForKey:@"id"]) value]:@"(none)"), subParser );
#endif
	...
	[SVGKLogger logMessage:@"[%@] WARN: found a tag with no namespace parser: (</%@>), using default parser(%@)", [self class], name, eventualParser];
...
#if DEBUG_XML_PARSER
	SVGKLog(@"[%@] tag: <%@:%@> id=%@ -- handled by subParser: %@", [self class], prefix, name, ([((Attr*)[attributeObjects objectForKey:@"id"]) value] != nil?[((Attr*)[attributeObjects objectForKey:@"id"]) value]:@"(none)"), eventualParser );
#endif
...
static void startElementSAX (void *ctx, const xmlChar *localname, const xmlChar *prefix,
	...
	SVGKLog(@"[%@] DEBUG_VERBOSE: <%@%@> (namespace URL:%@), attributes: %i", [self class], [NSString stringWithFormat:@"%@:",stringPrefix], name, stringURI, nb_attributes );
#endif
#endif
	...
	if( prefix2 == nil )
	{
		...
		if( nb_namespaces > 0 )
		{
			...
			{
				SVGKLog(@"[%@] DEBUG: found namespace [%i] : %@", [self class], i, namespaces[i] );
			}
		}
		else
			SVGKLog(@"[%@] DEBUG: there are ZERO namespaces!", [self class] );
		 */
	}
#endif
...
- (void)handleEndElement:(NSString *)name {
	//DELETE DEBUG SVGKLog(@"ending element, name = %@", name);
...
#if DEBUG_XML_PARSER
#if DEBUG_VERBOSE_LOG_EVERY_TAG
	SVGKLog(@"[%@] DEBUG-PARSER: ended tag (</%@>), handled by parser (%@) with parent parsed by %@", [self class], name, parser, parentParser );
#endif
#endif
...
static void errorEncounteredSAX (void *ctx, const char *msg, ...) {
	[SVGKLogger logMessage:@"Error encountered during parse: %s", msg];
	SVGKParser* self = getCurrentlyParsingParser();
...
{
	[SVGKLogger logMessage:@"Error: unparsed entity Decl"];
}
...
+(NSDictionary *) NSDictionaryFromCSSAttributes: (Attr*) styleAttribute {
	...
	{
		[SVGKLogger logMessage:@"[%@] WARNING: asked to convert an empty CSS string into a CSS dictionary; returning empty dictionary", [self class] ];
