...
-(void) reCalculateAndSetViewportElementReferenceUsingFirstSVGAncestor:(SVGElement*) firstAncestor
{
	...
	{
		[SVGKLogger logMessage:@"[%@] WARNING: setting self (tag = %@) to be a viewport", [self class], self.tagName];
	...
	else
	{
		...
		else
		{
			...
			if( newParent == nil )
			{
				 ...
				 */
				[SVGKLogger logMessage:@"SVGElement has had its parent set to nil; this makes the element and tree beneath it no-longer-valid SVG data; this may require fix-up if you try to re-add that SVGElement or any of its children back to an existing/new SVG tree"];
				self.rootOfCurrentDocumentFragment = nil;
			}
			else
			{
				...
				SVGKLog(@"viewport Element = %@ ... for node/element = %@", self.viewportElement, self.tagName);
#endif
			}
		}
	}
...
- (void)postProcessAttributesAddingErrorsTo:(SVGKParseResult *)parseResult  {
	...
	if( [[self getAttribute:@"transform"] length] > 0  || [[self getAttribute:@"gradientTransform"] length] > 0)
	{
		 ...
		 ^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop)
		{
			...
			//EXTREME DEBUG: SVGKLog(@"[%@] DEBUG: found a transform element (should be command + open bracket + args + close bracket) = %@", [self class], transformString);
			
			NSRange loc = [transformString rangeOfString:@"("];
			if( loc.length == 0 )
			{
				[SVGKLogger logMessage:@"[%@] ERROR: input file is illegal, has an item in the SVG transform attribute which has no open-bracket. Item = %@, transform attribute value = %@", [self class], transformString, value];
				return;
			}
			...
			//EXTREME DEBUG: SVGKLog(@"[%@] DEBUG: found parameters = %@", [self class], parameterStrings);
...
//					selfTransformable.transform = CGAffineTransformConcat( nt, selfTransformable.transform ); // Apple's method appears to be backwards, and not doing what Apple's docs state
					...
					{
						[SVGKLogger logMessage:@"[%@] ERROR: input file is illegal, has an SVG matrix transform attribute without the required 1 or 3 parameters. Item = %@, transform attribute value = %@", [self class], transformString, value];
					return;
				}
			}
		...
		//DEBUG: SVGKLog(@"[%@] Set local / relative transform = (%2.2f, %2.2f // %2.2f, %2.2f) + (%2.2f, %2.2f translate)", [self class], selfTransformable.transform.a, selfTransformable.transform.b, selfTransformable.transform.c, selfTransformable.transform.d, selfTransformable.transform.tx, selfTransformable.transform.ty );
		}
	}
